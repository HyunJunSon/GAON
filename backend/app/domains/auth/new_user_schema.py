...
class NewUserUpdate(BaseModel):
    ...
    def validate_new_password(cls, v):
        if v and not re.match(r'^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,16}$', v):
            auth_logger.warning(f"새 비밀번호의 형식이 유효하지 않습니다.")
            raise ValueError('새 비밀번호는 8~16자이며, 영문, 숫자, 특수문자를 각각 최소 1개 이상 포함해야 합니다.')
        return v
