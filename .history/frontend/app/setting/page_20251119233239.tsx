      ...
      <div className="space-y-8">
        ...
        <section className="rounded-2xl border border-orange-100 bg-white p-8 shadow-lg">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-orange-100 to-red-100 flex items-center justify-center text-orange-600">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            <div className="rounded-2xl border border-orange-100 bg-orange-50/60 p-6">
              <p className="text-sm font-medium text-orange-700">이름</p>
              <p className="mt-3 text-2xl font-semibold text-gray-900">{me?.name ?? '-'}</p>
            </div>
            <div className="rounded-2xl border border-orange-100 bg-orange-50/60 p-6">
              <p className="text-sm font-medium text-orange-700">이메일</p>
              <p className="mt-3 text-2xl font-semibold text-gray-900 break-all">{me?.email ?? '-'}</p>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-orange-100 bg-white p-8 shadow-lg">
          ...
          {isError && (
            <div className="rounded-2xl border border-red-100 bg-red-50/80 p-4 text-red-600">
              {(error as Error)?.message ?? '가족 구성원을 불러오지 못했어요.'}
            </div>
          )}
          ...
          {!!data?.members?.length && (
            <div className="grid gap-4">
              {data.members.map(m => (
                <div key={m.id} className="flex items-center justify-between rounded-2xl border border-gray-100 bg-gradient-to-r from-white to-orange-50/40 p-4 shadow-sm">
                  <div className="flex items-center gap-4">
                    <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-orange-100 to-red-100 text-orange-600">
                      <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                      </svg>
                    </div>
                    <div>
                      <p className="text-base font-semibold text-gray-900">{m.name ?? '이름 미입력'}</p>
                  ...
                  <button
                    ...
                    className="rounded-xl border border-red-200 bg-red-50 px-4 py-2 text-sm font-medium text-red-600 transition-colors hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-60"
        ...
        <section className="rounded-2xl border border-orange-100 bg-white p-8 shadow-lg">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-green-100 to-emerald-100 flex items-center justify-center text-green-700">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">메일을 입력하면 초대 메일이 발송됩니다.</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div>
              ...
              <input
                type="email"
                className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm focus:border-transparent focus:outline-none focus:ring-2 focus:ring-orange-500 transition"
            ...
            <button
              ...
              className="w-full rounded-xl bg-gradient-to-r from-orange-500 to-red-500 px-6 py-3 text-white font-semibold shadow-lg transition-all hover:from-orange-600 hover:to-red-600 hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-50"
            >
